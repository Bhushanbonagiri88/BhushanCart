...
function App() {
  ...
  return (
    <>
      ...
      <header className="py-2 shadow-sm fixed-top bg-white">
        <div className="container">
          <div className="d-flex flex-column flex-md-row align-items-center justify-content-between gap-2">
            ...
            >
              <div className="input-group">
                ...
                <button type="button" className="btn btn-outline-secondary d-flex align-items-center justify-content-center">
                  <VoiceAssistant products={products} triggerToast={triggerToast} />
                </button>
              </div>
            </form>
            ...
            {/* Toast Notification */}
            {showToast && (
              <div className="toast show position-fixed top-0 end-0 m-3" style={{ zIndex: 2000 }}>
                <div className="toast-body fw-bold">{toastMessage}</div>
              </div>
            )}
            ...
            <div className="d-flex align-items-center gap-2 flex-wrap justify-content-center">
              ...
              {isAuthenticated ? (
                ...
                <div className="d-flex gap-2">
                  <Link to="/login" className="btn btn-sm btn-outline-dark">🔑</Link>
                  <Link to="/register" className="btn btn-sm btn-outline-dark">👤</Link>
                </div>
              )}
            </div>
          </div>
      ...
      {/* ==================== Navbar ==================== */}
      <nav className="navbar navbar-expand-lg navbar-dark fixed-top shadow-sm" style={{ background: "linear-gradient(90deg, #6a11cb, #2575fc)", top: "60px" }}>
        <div className="container-fluid">
          <button
            className="navbar-toggler"
            type="button"
            ...
            <ul className="navbar-nav mx-auto text-center">
              {navLinks.map((link, i) => (
                <li className="nav-item" key={i}>
                  <Link
                    className={`nav-link fw-semibold px-3 py-2 ${
                      location.pathname === link.path ? "active-nav text-dark rounded" : "text-white"
                    }`}
                    to={link.path}
                    onClick={() => setIsNavCollapsed(true)}
                  >
      ...
      {/* ==================== Main Content ==================== */}
      <main style={{ marginTop: "140px", padding: "1rem" }}>
        <Routes>
          <Route path="/" element={<Home />} />
          <Route path="/kitchen" element={<Kitchen />} />
          <Route path="/kitchen/veg" element={<Veg />} />
